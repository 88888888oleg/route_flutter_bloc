...
import 'package:route_flutter_bloc/src/route_bloc_provider.dart';
  ...
  Widget build(BuildContext context) {
    return RouteBlocListener<CounterCubit, int>(
      ...
      listener: (context, state) {
        print('LISTENER TRIGGERED: $state');
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Listener: $state')),
        );
      },
      child: Scaffold(
        appBar: AppBar(title: const Text('Page 1 - Listener ACTIVE')),
        body: Center(
          child: Column(
            ...
            children: [
              ...
              Row(
                ...
                children: [
                  Checkbox(
                    value: triggerOnResumed,
                    onChanged: (value) {
                      setState(() => triggerOnResumed = value ?? false);
                    },
                  ),
                  const Text('Trigger on resume'),
                ],
              ),
              const SizedBox(height: 10),
              Row(
      ...
      body: Center(
        child: Column(
          ...
          children: [
            ...
            ElevatedButton(
              onPressed: () => context.read<CounterCubit>().increment(),
              child: const Text('Increment (listener should NOT fire)'),
            ),
            const SizedBox(height: 10),
            ElevatedButton(
